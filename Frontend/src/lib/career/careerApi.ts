...
export async function createResumeVersion(payload: {
  ...
  if (isStaticPrototype()) {
    return {
      ...
      filePath: `/uploads/resumes/${payload.fileName}`,
  ...
  return requestData<ResumeVersion>("/api/career/resumes", {
    method: "POST",
...
export async function listResumeVersions() {
  if (isStaticPrototype()) {
    return { items: [await createResumeVersion({ fileName: "static-resume.txt", extractedText: "React TypeScript project" })] };
  }
  return requestData<{ items: ResumeVersion[] }>("/api/career/resumes");
}
...
export async function mergeResumeToProfile(resumeVersionId: string) {
  return requestData<{ updated: boolean; profile: CareerProfile; mergedSkills: string[] }>(
    `/api/career/resumes/${encodeURIComponent(resumeVersionId)}/merge-to-profile`,
    { method: "POST" }
  );
}

export async function getOpportunityFit(opportunityId: string, resumeVersionId?: string) {
  if (isStaticPrototype()) {
    return {
      fitScore: 78,
      breakdown: {
        eligibilityScore: 1,
        skillMatchScore: 0.67,
        resumeQualityScore: 0.72,
        projectEvidence: 1,
      },
      matchedSkills: ["React", "TypeScript"],
      missingSkills: ["Node.js"],
      eligibility: { eligible: true, branchEligible: true, yearEligible: true },
      reasons: ["Meets listed branch/year eligibility.", "Matches 2 required skills.", "Uses latest resume version."],
      recommendations: ["Close skill gaps: Node.js.", "Add project bullets with measurable outcomes."],
      resumeVersionId: resumeVersionId || "resume-static",
      opportunityId,
    } satisfies OpportunityFit;
  }
  const params = new URLSearchParams();
  if (resumeVersionId) params.set("resumeVersionId", resumeVersionId);
  const query = params.toString() ? `?${params.toString()}` : "";
  return requestData<OpportunityFit>(`/api/career/opportunities/${encodeURIComponent(opportunityId)}/fit${query}`);
}
