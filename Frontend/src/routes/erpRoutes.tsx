...
const ResumeBuilder = lazy(() => import("../pages/CareerPortal/ResumeBuilder"));

/**
 * Bespoke components mounted at a blueprint's route in place of the generic
 * `BlueprintPage`. A key here only becomes a live route if a **visible**
 * blueprint owns the same route (see the `Object.values(PAGE_BLUEPRINTS)`
 * pipeline below) — `routeReachability.test.tsx` asserts that invariant so a
 * key can't silently rot into an unreachable page.
 */
export const DOMAIN_PAGE_MAP: Record<string, React.ReactNode> = {
