...
export default function RaiseTicket() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setBanner({
        tone: "warning",
        text: error instanceof Error ? error.message : "Failed to submit ticket.",
      });
    }
  }
              ...
              <div key={ticket.id} className="rounded-2xl border border-[var(--border)] bg-white p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="text-right text-xs text-[var(--text-secondary)]">
                    ...
                    <div className="mt-1">Assigned: {ticket.assignedTo}</div>
                    <div className="mt-1">Updated: {new Date(ticket.updatedAt).toLocaleString("en-IN")}</div>
                  </div>
                </div>
              </div>
            ))}
