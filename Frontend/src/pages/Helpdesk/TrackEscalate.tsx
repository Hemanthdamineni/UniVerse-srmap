    ...
    >
      ...
      <SectionCard title={adminMode && admin.unlocked ? "All Tickets" : "Your Tickets"}>
        ...
        {adminMode && admin.unlocked ? (
          <div className="grid grid-cols-[minmax(0,1fr)] gap-2 lg:grid-cols-[minmax(0,1fr)_auto]">
            <div className="flex flex-wrap gap-2 text-xs text-[var(--comp-text-secondary)]">
              {workload.slice(0, 4).map((item) => (
                <span key={`${item.assignedTeam}-${item.ownerName}`} className="rounded-full border border-[var(--comp-border)] px-3 py-1">
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex-1 space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full border border-[var(--comp-border)] px-2 py-1 text-xs font-semibold text-[var(--comp-text-secondary)]">
                    ...
                    <div className="grid gap-1 text-xs text-[var(--comp-text-secondary)] md:grid-cols-3">
                    ...
                    {ticket.replies?.length ? (
                      <div className="bg-[color-mix(in_srgb,var(--comp-surface-hover)_55%,transparent)] p-3">
                        <h4 className="text-xs font-semibold uppercase tracking-wide text-[var(--comp-text-secondary)]">
                          Conversation
                        </h4>
                        <div className="divide-y divide-[var(--comp-border)]">
                          {ticket.replies.slice(0, 3).map((reply) => (
                            <div key={reply.id} className="py-2 text-sm">
                              <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-[var(--comp-text-secondary)]">
                                <span>
                                  {reply.authorName} · {reply.authorRole}
                                </span>
                                <span>{new Date(reply.createdAt).toLocaleString("en-IN")}</span>
                              </div>
                              <div className="mt-1 text-[var(--comp-text-secondary)]">
                    ...
                    {adminMode && admin.unlocked && ticket.auditTrail?.length ? (
                      <div className="text-xs text-[var(--comp-text-secondary)]">
                    ...
                    {adminMode && admin.unlocked ? (
                      <>
                        ...
                        <input
                          ...
                          className="rounded-lg border border-[var(--comp-border)] bg-[var(--comp-surface)] px-3 py-2 min-h-11 text-sm outline-none focus:border-[var(--comp-accent)]"
                        />
                        <button
                          type="button"
