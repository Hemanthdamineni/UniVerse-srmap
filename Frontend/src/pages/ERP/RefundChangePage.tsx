...
export default function RefundChangePage({ blueprint }: Props) {
  ...
  useEffect(() => {
    ...
    setError(batchQuery.error instanceof Error ? batchQuery.error.message : "Failed to load refund & change requests.");
  }, [batchQuery.error]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setError(err.message || "Failed to load refund & change requests.");
    }
