        ...
        <SectionCard title="Add Event to Feedback List">
          ...
          {eventOptions.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {eventOptions.map((e) => (
                <span key={e.id} className="rounded-full bg-[#0A3035]/8 px-3 py-1 text-xs font-semibold text-[#0A3035]">{e.name}</span>
              ))}
            </div>
          )}
        </SectionCard>
            ...
            {entries.map((entry) => (
              <div key={entry.id} className="rounded-2xl border border-[var(--border)] bg-white p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h3 className="text-base font-semibold text-[#0A3035]">{entry.eventName}</h3>
