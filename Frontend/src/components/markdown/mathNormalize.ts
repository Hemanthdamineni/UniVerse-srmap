/**
 * mathNormalize.ts — the dependency-free half of the math pipeline.
 *
 * `normalizeMathSource` promotes Pandoc `\\(…\\)` / `\\[…\\]` to `$…$` / `$$…$$`,
 * fences standalone display math, and escapes money-like `$…$` pairs. It runs
 * synchronously on every render, so it must never pull KaTeX — that lives in
 * the lazily-imported `./math`.
 */
