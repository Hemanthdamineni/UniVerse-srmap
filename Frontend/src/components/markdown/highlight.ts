...
// yaml, sql, c/cpp, java, csharp…) fall back to plain <pre>-style token
// streams — they don't produce a Tree, but the same `tok-*` stylesheet
// paints both paths.
const loaders: Record<string, ParserLoader> = {
  ...
  // `phpLanguage` parses files wrapped in `<?php …`. Snippets are bare PHP,
  // so use the `plain: true` variant's Language.
  php: () =>
