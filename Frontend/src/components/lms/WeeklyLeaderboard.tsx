export default function WeeklyLeaderboard({
  ...
  return (
    <section className="dashboard-card space-y-3 p-4">
      <h3 className="text-lg font-semibold text-[#0A3035]">Weekly leaderboard</h3>
      <div className="space-y-2">
        {items.map((item, index) => (
          <div key={`${item.userId}-${index}`} className="flex items-center justify-between rounded-2xl bg-white/80 px-3 py-2">
            <span className="text-sm font-medium text-[#0A3035]">#{index + 1} {String(item.userId || "unknown")}</span>
