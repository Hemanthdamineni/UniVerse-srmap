...
| **Real-ERP rejection wording fixed** (found via live diagnostics): the ERP writes rejections to `<div id="divmsg">` using "Captcha Invalid" — the reverse of the phrase we historically matched, hidden past the 6 KB artifact truncation. Classifier now reads `#divmsg` first (captcha / credentials / unknown-passthrough verbatim); diagnostic capture raised to 40 KB | `erpClient.js`, `config/env.js` |
