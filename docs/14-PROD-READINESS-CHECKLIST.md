...
| ☐ | P1 | One coherent deployment topology documented and rehearsed: root compose (backend+redis) + monitoring override + ingress serving `Frontend/dist` with `/api` proxy. Decide: ingress container (then wire TLS certs/443 into `compose.ingress.yml`) vs host nginx (`setup-tls.sh` path) — pick one, delete the other's ambiguity | architecture note + `docker compose config` output |
