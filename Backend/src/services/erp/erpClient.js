const { BASE_ORIGIN, BASE_PATH, LOGIN_URL, LOGIN_PREAUTH_TTL_MS, LOGIN_POST_URL } = require("../../config/env");
const { createLoginAttemptId, createLoginAttemptTrace } = require("../core/sessionServices");
const { request, chromium } = require("playwright");
const cheerio = require("cheerio");
const { cleanText } = require("../../utils/text");
const fs = require("fs");
const path = require("path");
const { getSubitemExtractor, adaptToLegacyPayload } = require("./extractors");

// --- authProbe.js (utility) ---
...
async function callEndpointViaApi(api, endpoint, menuItem, variables = null, bodyOverride = null) {
  ...
  // Check for session expiry against raw HTML — fast regex, no full parse needed
  if (isErpSessionExpiredResponse(body, null)) {
    throw makeSessionExpiredError();
  }

  const dropdown = menuItem?.dropdown;
  const subitem = menuItem?.subitem;
  const extractorMatch = getSubitemExtractor(dropdown, subitem);

  if (extractorMatch) {
    const extracted = extractorMatch.extractor(body);
    const legacyPayload = adaptToLegacyPayload(extracted);

    return {
      ...legacyPayload,
      status: bodyOverride ? 200 : response.status(),
      rawHtml: body,
      endpoint: { method, url, params: formParams },
    };
  }

  // Every ERP page in scrapeTargets.js MUST have a registered extractor.
  // If this throws, a page was added without a corresponding extractor.
  const unregisteredErr = new Error(
    `[erpExtractors] UNREGISTERED PAGE: No targeted extractor for "${dropdown}|${subitem}". ` +
    `Add an extractor to Backend/src/services/erp/extractors/ and register it in index.js.`
  );
  unregisteredErr.code = "UNREGISTERED_ERP_PAGE";
  unregisteredErr.status = 500;
  unregisteredErr.context = { dropdown, subitem };
  throw unregisteredErr;
}
...
module.exports = {
  ...
  setCaptureDir,
  redactSensitiveText: require("../core/sessionServices").redactSensitiveText,
  sanitizeArtifactPayload: require("../core/sessionServices").sanitizeArtifactPayload,
};
