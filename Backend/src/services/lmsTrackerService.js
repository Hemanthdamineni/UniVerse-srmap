...
class LmsTrackerService {
  constructor({ erpAggregationService }) {
  ...
  async getOverview({ sessionId }) {
    const { examMarkRaw, currentRaw, attendanceRaw, cgpaRaw } = await this._loadBatch(sessionId);
  ...
  async getInsights({ sessionId }) {
    const overview = await this.getOverview({ sessionId });
    const { examMarkRaw, currentRaw } = await this._loadBatch(sessionId);
