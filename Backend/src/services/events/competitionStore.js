...
const schemaMethods = {
  _ensureSchema() {
    ...
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS team_recruitment_posts (
        id TEXT PRIMARY KEY,
        eventId TEXT NOT NULL,
        teamId TEXT NOT NULL UNIQUE,
        createdBy TEXT NOT NULL,
        status TEXT NOT NULL DEFAULT 'open',
        neededSkills TEXT DEFAULT '[]',
        description TEXT,
        openSlots INTEGER DEFAULT 1,
        createdAt TEXT NOT NULL,
        updatedAt TEXT NOT NULL
      );
    `);
    this.db.exec("CREATE INDEX IF NOT EXISTS idx_team_recruitment_event ON team_recruitment_posts(eventId, status);");
    this.db.exec(`
...
const teamMethods = {
  ...
  _publicTeam(team) {
    ...
    const pending = this.db
      .prepare(
        "SELECT inviteeRegisterNumber, createdAt FROM team_invitations WHERE eventId = ? AND teamId = ? AND status = 'pending'"
      )
      .all(team.eventId, team.id);
    const pendingIds = new Set(pending.map((item) => item.inviteeRegisterNumber));
    const members = [
      ...team.members.map((regNo) => ({
        regNo,
        name: regNo,
      ...
      ...pending.map((item) => ({
        regNo: item.inviteeRegisterNumber,
        name: item.inviteeRegisterNumber,
        joinedAt: item.createdAt,
        status: "pending",
      })),
    ].filter((member, index, all) =>
      member.status === "pending" || !pendingIds.has(member.regNo) || all.findIndex((item) => item.regNo === member.regNo) === index
    );
    return {
      ...team,
      leaderRegNo: team.leaderId,
      members,
      memberRegNos: team.members,
    };
  },

  listEventTeams(eventId, user) {
    const event = this._getEventOrThrow(eventId);
    this._requireTeamScopedEvent(event);
    const rows = this.db
      .prepare("SELECT * FROM teams WHERE eventId = ? ORDER BY createdAt ASC")
      .all(eventId);
    return rows.map((row) => this._publicTeam(this._hydrateTeam(row)));
  },

  upsertTeamRecruitmentPost(eventId, userId, payload = {}) {
    const event = this._getEventOrThrow(eventId);
    this._requireTeamScopedEvent(event);
    const team = this.getMyTeam(eventId, userId);
    if (!team) {
      const error = new Error("Create or join a team before posting recruitment needs.");
      error.status = 404;
      throw error;
    }
    if (team.leaderId !== userId) {
      const error = new Error("Only the team leader can publish recruitment needs.");
      error.status = 403;
      throw error;
    }
    const maxTeamSize = this._getMaxTeamSize(event);
    const availableSlots = Math.max(0, maxTeamSize - team.members.length);
    const requestedSlots = payload.openSlots ?? (availableSlots || 1);
    const openSlots = Math.min(
      availableSlots,
      Math.max(0, Math.floor(Number(requestedSlots)))
    );
    const neededSkills = normalizeSkillList(payload.neededSkills);
    const description = String(payload.description || "").trim().slice(0, 500);
    const status = String(payload.status || (openSlots > 0 ? "open" : "closed")).toLowerCase() === "closed" ? "closed" : "open";
    const existing = this.db.prepare("SELECT id, createdAt FROM team_recruitment_posts WHERE teamId = ?").get(team.id);
    const now = nowIso();
    const post = {
    ...
    this.db
      .prepare(
        `INSERT INTO team_recruitment_posts (
          id, eventId, teamId, createdBy, status, neededSkills, description, openSlots, createdAt, updatedAt
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(teamId) DO UPDATE SET
          status = excluded.status,
          neededSkills = excluded.neededSkills,
          description = excluded.description,
          openSlots = excluded.openSlots,
          updatedAt = excluded.updatedAt`
      )
      .run(
        post.id,
        ...
        post.createdBy,
        post.status,
        JSON.stringify(post.neededSkills),
        post.description,
        post.openSlots,
        post.createdAt,
        post.updatedAt
      );
    return this._hydrateRecruitmentPost({ ...post, neededSkills: JSON.stringify(post.neededSkills) });
  },

  _hydrateRecruitmentPost(row) {
    if (!row) return null;
    const team = this._publicTeam(this._hydrateTeam(this.db.prepare("SELECT * FROM teams WHERE id = ?").get(row.teamId)));
    return {
      ...row,
      neededSkills: safeJsonParse(row.neededSkills, []),
      team,
    };
  },

  listTeamRecruitmentBoard(eventId, user) {
    const event = this._getEventOrThrow(eventId);
    this._requireTeamScopedEvent(event);
    const rows = this.db
      .prepare(
        `SELECT * FROM team_recruitment_posts
         WHERE eventId = ? AND status = 'open'
         ORDER BY updatedAt DESC`
      )
      .all(eventId);
    const myTeam = this.getMyTeam(eventId, user.userId);
    return rows
      .map((row) => this._hydrateRecruitmentPost(row))
      .filter((post) => post?.team && post.teamId !== myTeam?.id && Number(post.openSlots || 0) > 0);
  },

  listTeamMatches(eventId, user) {
    const event = this._getEventOrThrow(eventId);
    this._requireTeamScopedEvent(event);
    const myTeam = this.getMyTeam(eventId, user.userId);
    const myPost = myTeam
      ? this.db.prepare("SELECT * FROM team_recruitment_posts WHERE teamId = ?").get(myTeam.id)
      : null;
    const desiredSkills = normalizeSkillList(safeJsonParse(myPost?.neededSkills, []));
    const registrations = (this.eventsStore.registrationsByEvent.get(eventId) || [])
      .filter((item) => item.status === "registered" && item.userId !== user.userId)
      .filter((item) => !this.getMyTeam(eventId, item.userId));
    return registrations
      .map((registration) => {
        const responseText = JSON.stringify(registration.formResponses || []).toLowerCase();
        const matchedSkills = desiredSkills.filter((skill) => responseText.includes(skill.toLowerCase()));
        const sameDepartment = registration.department && user.department && registration.department === user.department;
        const score = Math.min(100, Math.round(matchedSkills.length * 28 + (sameDepartment ? 12 : 0) + 35));
        return {
          userId: registration.userId,
          name: registration.userName || registration.userId,
          department: registration.department || "",
          matchedSkills,
          missingSkills: desiredSkills.filter((skill) => !matchedSkills.includes(skill)),
          matchScore: score,
          reasons: [
            matchedSkills.length ? `Matches ${matchedSkills.length} needed skill${matchedSkills.length === 1 ? "" : "s"}` : "Registered and not yet on a team",
            sameDepartment ? "Same department context" : "Available for team formation",
          ],
        };
      })
